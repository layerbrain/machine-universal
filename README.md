...
- **Containers**: Docker Engine, Docker Compose v2. The runtime starts Docker automatically, runs Compose with `--parallel 1` by default for nested-container stability, reaps orphaned processes with `tini`, and restarts `dockerd` if it exits.
- **Editors**: vim, nano
