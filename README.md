...
- **Containers**: Docker Engine, Docker Compose v2. The runtime starts Docker automatically, hard-serializes detached Compose `up` starts for nested-container stability, reaps orphaned processes with `tini`, and restarts `dockerd` if it exits.
- **Editors**: vim, nano
